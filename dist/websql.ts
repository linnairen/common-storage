...
export namespace CommonWebSql {
  ...
  export class WebSqlDb {
    ...
    select = async (tablename: string, columns:string[] = [], selectors?: Array<{ key: string, value: string }>) => {
      ...
      if (selectors) {
        qStr += ` where ${selectors.map(it => `${it.key}="${it.value}"`).join(' ')}`
      }
    ...
    add = async (tablename: string, data: Array<{ key: string, value: string }>) => {
      ...
      if (TypeVerify.IsArray(data)) {
        ...
        qStr += ` (${keys.join(',')}) values (${values.map(it => `"${it}"`).join(',')})`
      }
      return await this.DealSql(qStr)
    }
    /**
     * 更新数据
     * @param tablename 要更新数据的表格名
     * @param data 更新数据 格式为：[ { key: "name", value: "Bob" } ]
     * @param selectors 查询条件组合（暂时只支持等于 如：name = "Bob"）
     */
    update = async (tablename: string, data: Array<{ key: string, value: string }>, selectors: Array<{ key: string, value: string }>) => {
      if (!tablename) { throw new TypeError('tablename is requested') }
      if (!data) { throw new TypeError('data is requested') }
      if (!selectors) { throw new TypeError('selectors is requested') }
      let qStr = `update ${tablename} set `
      if (TypeVerify.IsArray(data)) {
        qStr += data.map(it => `${it.key}="${it.value}"`).join(',')
      }
      if (TypeVerify.IsArray(selectors)) {
        qStr += ` where ${selectors.map(it => `${it.key}="${it.value}"`).join(' ')}`
      }
      return await this.DealSql(qStr)
    }
    /**
     * 删除数据
     * @param tablename 要删除数据的表格名
     * @param selectors 查询条件组合（暂时只支持等于 如：name = "Bob"）
     */
    delete = async (tablename: string, selectors: Array<{ key: string, value: string }>) => {
      if (!tablename) { throw new TypeError('tablename is requested') }
      if (!selectors) { throw new TypeError('selectors is requested') }
      let qStr = `delete from ${tablename}`
      if (TypeVerify.IsArray(selectors)) {
        qStr += ` where ${selectors.map(it => `${it.key}="${it.value}"`).join(' ')}`
      }
      return await this.DealSql(qStr)
